...
const COMMON: LazyLock<TokenStream2> = LazyLock::new(|| {
    quote! {
        fn recog(arg: &mut ParseArgs) -> <Self as CommonTypes>::Output {
            if let Some(e) = Self::check_fail(arg) {
                Err(e)
            } else {
                Self::check_pass(arg).map(|(_, s)| Ok(s)).unwrap_or_else(|| Self::parse(arg))
            }
        }
    }
});
...
fn check_pass_fail(name: &str, t: impl ToTokens) -> TokenStream2 {
    ...
    quote! {
        fn check_pass(arg: &mut ParseArgs) -> Option<(usize, Self)> {
            ...
            arg.c_a_d.borrow().cache.pass.iter().enumerate().find_map(|(i, k)| {
                k.items.get(k.index).and_then(|v| {
                    (v.0 == Self::CONST && v.1 == when_not_fail).then(|| {
                        ...
                        arg.print.from_cache::<true>(#literal, Self::CONST);
                        (i, v.clone())
                    })
                })
            })
        }
        fn check_fail(arg: &mut ParseArgs) -> Option<<Self as CommonTypes>::Error> {
            arg.c_a_d.borrow().cache.fails.get(&(Self::CONST, arg.code.cursor)).map(|e| {
                arg.print.from_cache::<false>(#literal, Self::CONST);
                e
            }).cloned()
        }
    }
